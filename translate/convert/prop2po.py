...
class prop2po:
    ...
    def convertstore(self, thepropfile):
        ...
        self.mixednames = self.mixer.match_entities(thepropfile.index)
    ...
    def mergestore(self, origpropfile, translatedpropfile):
        ...
        #TODO: self.mixednames is overwritten below, so this is useless:
        self.mixednames = self.mixer.match_entities(origpropfile.index)
        translatedpropfile.makeindex()
        self.mixednames = self.mixer.match_entities(translatedpropfile.index)
    ...
    def convertpropunit(self, store, unit, commenttype, mixbucket="dtd"):
        ...
        labelname, accesskeyname = self.mixer.find_mixed_pair(self.mixednames, store, unit)
        labelprop = store.index.get(labelname, None)
        accesskeyprop = store.index.get(accesskeyname, None)
        po_unit = self.convertmixedunit(labelprop, accesskeyprop, commenttype)
        if po_unit is not None:
            if accesskeyname is not None:
                self.mixednames[accesskeyname][mixbucket] = True
            if labelname is not None:
                self.mixednames[labelname][mixbucket] = True
            return po_unit
        else:
            # otherwise the mix failed. add each one separately and
            # remember they weren't mixed
            if accesskeyname is not None:
                self.mixednames[accesskeyname][mixbucket] = False
            if labelname is not None:
                self.mixednames[labelname][mixbucket] = False
